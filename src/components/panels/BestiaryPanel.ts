...
export class BestiaryPanel extends ItemView {
  ...
  refreshLocalization = () => {
    ...
    if (this.searchInput) {
      this.searchInput.setAttribute(
        "placeholder",
        i18n.t("BESTIARY.SEARCH_PLACEHOLDER"),
      );
    }
  ...
  private filterCreatures() {
    ...
    if (creaturesList) {
      ...
      const groupedCreatures = this.groupCreaturesByFirstLetter(
        sortedFilteredCreatures,
      );
      this.renderGroupedCreaturesList(
        creaturesList as HTMLElement,
        groupedCreatures,
      );
    }
  }

  private groupCreaturesByFirstLetter(
    creatures: Creature[],
  ): Map<string, Creature[]> {
  ...
  private renderGroupedCreaturesList(
    container: HTMLElement,
    groupedCreatures: Map<string, Creature[]>,
  ) {
    const sortedLetters = Array.from(groupedCreatures.keys()).sort();

    sortedLetters.forEach((letter) => {
      const creatures = groupedCreatures.get(letter)!;
      const letterSection = container.createDiv({
        cls: "bestiary-letter-section",
      });
      ...
      const creaturesContainer = letterSection.createDiv({
        cls: "bestiary-creatures-container",
      });
  ...
  private renderCreatureListItem(container: HTMLElement, creature: Creature) {
    ...
    const checkboxContainer = creatureEl.createDiv({
      cls: "creature-checkbox-container",
    });
    ...
    const detailsRow = creatureContent.createDiv({
      cls: "creature-details-row",
    });
  ...
  private async handleDeleteClick() {
    ...
    const confirmMessage =
      this.selectedCreatures.size === 1
        ? i18n.t("BESTIARY.DELETE_CONFIRM_SINGLE", { name: selectedNames[0] })
        : i18n.t("BESTIARY.DELETE_CONFIRM_MULTIPLE", {
            count: selectedNames.length.toString(),
          });
    ...
    if (successCount > 0) {
      const successMessage =
        successCount === 1
          ...
          : i18n.t("BESTIARY.DELETE_SUCCESS_MULTIPLE", {
