...
export class AbilityScoresComponent {
  ...
  private saving_throws_proficiency: boolean[] = [
    false,
    false,
    false,
    false,
  ...
  private renderAbilities(container: HTMLElement) {
    const abilitiesContainer = container.createDiv({
    ...
    const abilities = [
      {
        index: 0,
        label: i18n.t("ABILITY_SCORES.ABILITIES.STR"),
        fullName: i18n.t("ABILITY_SCORES.ABILITIES.STR_FULL"),
      },
      {
        index: 1,
        label: i18n.t("ABILITY_SCORES.ABILITIES.DEX"),
        fullName: i18n.t("ABILITY_SCORES.ABILITIES.DEX_FULL"),
      },
      {
        index: 2,
        label: i18n.t("ABILITY_SCORES.ABILITIES.CON"),
        fullName: i18n.t("ABILITY_SCORES.ABILITIES.CON_FULL"),
      },
      {
        index: 3,
        label: i18n.t("ABILITY_SCORES.ABILITIES.INT"),
        fullName: i18n.t("ABILITY_SCORES.ABILITIES.INT_FULL"),
      },
      {
        index: 4,
        label: i18n.t("ABILITY_SCORES.ABILITIES.WIS"),
        fullName: i18n.t("ABILITY_SCORES.ABILITIES.WIS_FULL"),
      },
      {
        index: 5,
        label: i18n.t("ABILITY_SCORES.ABILITIES.CHA"),
        fullName: i18n.t("ABILITY_SCORES.ABILITIES.CHA_FULL"),
      },
    ];

    abilities.forEach((ability) =>
      this.renderAbilityColumn(abilitiesContainer, ability),
    );
  }
  ...
  private renderAbilityColumn(container: HTMLElement, ability: any) {
    ...
    const modifierInput = abilityCol.createEl("input", {
      type: "text",
      value: this.formatModifier(
        this.calculateModifier(this.characteristics[ability.index]),
      ),
  ...
  private renderSavingThrows(container: HTMLElement) {
    ...
    const savingThrowsContainer = container.createDiv({
    ...
    const savingThrows = [
      {
        index: 0,
        label: i18n.t("ABILITY_SCORES.ABILITIES.STR"),
        fullName: `${i18n.t("ABILITY_SCORES.SAVING_THROWS")} ${i18n.t("ABILITY_SCORES.ABILITIES.STR_FULL")}`,
      },
      {
        index: 1,
        label: i18n.t("ABILITY_SCORES.ABILITIES.DEX"),
        fullName: `${i18n.t("ABILITY_SCORES.SAVING_THROWS")} ${i18n.t("ABILITY_SCORES.ABILITIES.DEX_FULL")}`,
      },
      {
        index: 2,
        label: i18n.t("ABILITY_SCORES.ABILITIES.CON"),
        fullName: `${i18n.t("ABILITY_SCORES.SAVING_THROWS")} ${i18n.t("ABILITY_SCORES.ABILITIES.CON_FULL")}`,
      },
      {
        index: 3,
        label: i18n.t("ABILITY_SCORES.ABILITIES.INT"),
        fullName: `${i18n.t("ABILITY_SCORES.SAVING_THROWS")} ${i18n.t("ABILITY_SCORES.ABILITIES.INT_FULL")}`,
      },
      {
        index: 4,
        label: i18n.t("ABILITY_SCORES.ABILITIES.WIS"),
        fullName: `${i18n.t("ABILITY_SCORES.SAVING_THROWS")} ${i18n.t("ABILITY_SCORES.ABILITIES.WIS_FULL")}`,
      },
      {
        index: 5,
        label: i18n.t("ABILITY_SCORES.ABILITIES.CHA"),
        fullName: `${i18n.t("ABILITY_SCORES.SAVING_THROWS")} ${i18n.t("ABILITY_SCORES.ABILITIES.CHA_FULL")}`,
      },
    ];
    ...
    savingThrows.forEach((savingThrow) => {
      ...
      const savingThrowInput = savingThrowCol.createEl("input", {
        type: "text",
        value: this.formatModifier(
          this.calculateSavingThrowValue(savingThrow.index),
        ),
  ...
  private toggleSavingThrowProficiency(
    abilityIndex: number,
    inputElement: HTMLInputElement,
  ) {
    this.saving_throws_proficiency[abilityIndex] =
      !this.saving_throws_proficiency[abilityIndex];
    ...
    if (this.saving_throws_proficiency[abilityIndex]) {
      ...
      inputElement.title = inputElement.title.replace(
        ` (${i18n.t("ABILITY_SCORES.PROFICIENCY_HINT")})`,
        "",
      );
    }

    inputElement.value = this.formatModifier(
      this.calculateSavingThrowValue(abilityIndex),
    );
  }
  ...
  private calculateSavingThrowValue(abilityIndex: number): number {
    const baseModifier = this.calculateModifier(
      this.characteristics[abilityIndex],
    );
    if (this.saving_throws_proficiency[abilityIndex]) {
      return baseModifier + this.proficiency_bonus;
    }
  ...
  private updateSavingThrowsDisplay(): void {
    ...
    savingThrowInputs.forEach((input, index) => {
      if (index < this.saving_throws_proficiency.length) {
        (input as HTMLInputElement).value = this.formatModifier(
          this.calculateSavingThrowValue(index),
        );
      }
    });
  }
  ...
  getSavingThrowsProficiency(): boolean[] {
    return this.saving_throws_proficiency;
  }
