...
export class CreatureCreationModal extends Modal {
  ...
  private async saveCreature() {
    ...
    const creatureData = {
      ...
      alignment: i18n.getAlignment(
        this.basicFields.getAlignment() as AlignmentKey,
      ),
      ...
      damage_resistances: this.immunities
        .getDamageResistances()
        .map((res) => i18n.getDamageType(res as any)),
      damage_vulnerabilities: this.immunities
        .getDamageVulnerabilities()
        .map((vul) => i18n.getDamageType(vul as any)),
      damage_immunities: this.immunities
        .getDamageImmunities()
        .map((imm) => i18n.getDamageType(imm as any)),
      condition_immunities: this.immunities
        .getConditionImmunities()
        .map((cond) => i18n.getCondition(cond as any)),
