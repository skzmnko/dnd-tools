...
export class BestiaryService {
  ...
  async initialize(): Promise<void> {
    ...
    console.log(
      "BestiaryService initialized with",
  ...
  async createCreature(
    creatureData: Omit<Creature, "id" | "created" | "updated">,
  ): Promise<Creature> {
    if (!this.isInitialized) {
    ...
    try {
      ...
      console.log(
        "Creature created successfully, total creatures:",
        this.creatures.length,
      );
      return creature;
    } catch (error) {
  ...
  generateId(): string {
    return (
      "creature_" + Date.now() + "_" + Math.random().toString(36).substr(2, 9)
    );
  }
}
